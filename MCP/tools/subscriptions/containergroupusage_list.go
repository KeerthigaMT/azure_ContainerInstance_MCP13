...
func Containergroupusage_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/providers/Microsoft.ContainerInstance/locations/%s/usages%s", cfg.BaseURL, subscriptionId, location, queryString)
...
func CreateContainergroupusage_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_providers_Microsoft.ContainerInstance_locations_location_usages",
		mcp.WithDescription("Get the usage for a subscription"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Containergroupusage_listHandler(cfg),
	}
}
