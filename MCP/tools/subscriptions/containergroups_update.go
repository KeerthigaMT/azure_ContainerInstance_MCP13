...
func Containergroups_updateHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateContainergroups_updateTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.ContainerInstance_containerGroups_containerGroupName",
		mcp.WithDescription("Update container groups."),
		...
		mcp.WithObject("Resource", mcp.Required(), mcp.Description("The container group resource with just the tags to be updated.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Containergroups_updateHandler(cfg),
	}
}
