...
func ListcapabilitiesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/providers/Microsoft.ContainerInstance/locations/%s/capabilities%s", cfg.BaseURL, subscriptionId, location, queryString)
...
func CreateListcapabilitiesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_providers_Microsoft.ContainerInstance_locations_location_capabilities",
		mcp.WithDescription("Get the list of capabilities of the location."),
		...
		mcp.WithString("location", mcp.Required(), mcp.Description("The identifier for the physical azure location.")),
		mcp.WithString("api-version", mcp.Required(), mcp.Description("Client API version")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListcapabilitiesHandler(cfg),
	}
}
