...
func Containergroups_listHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/providers/Microsoft.ContainerInstance/containerGroups%s", cfg.BaseURL, subscriptionId, queryString)
...
func CreateContainergroups_listTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_providers_Microsoft.ContainerInstance_containerGroups",
		mcp.WithDescription("Get a list of container groups in the specified subscription."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Containergroups_listHandler(cfg),
	}
}
