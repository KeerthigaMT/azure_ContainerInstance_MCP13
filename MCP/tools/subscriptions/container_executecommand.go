...
func Container_executecommandHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/subscriptions/%s/resourceGroups/%s/providers/Microsoft.ContainerInstance/containerGroups/%s/containers/%s/exec%s", cfg.BaseURL, subscriptionId, resourceGroupName, containerGroupName, containerName, queryString)
...
func CreateContainer_executecommandTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft.ContainerInstance_containerGroups_containerGroupName_containers_containerName_exec",
		mcp.WithDescription("Executes a command in a specific container instance."),
		...
		mcp.WithObject("containerExecRequest", mcp.Required(), mcp.Description("The request for the exec command.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Container_executecommandHandler(cfg),
	}
}
